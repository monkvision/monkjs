 ...
 * Uses the `advanced` constraints array which is the correct way to apply optional constraints
 * that should be silently ignored if unsupported (e.g. iOS Safari < 17).
 *
 * Compatible with:
 * - Android Chrome: full support
 * - iOS Safari 17+: supported
 * - iOS Safari < 17: silently ignored (no crash)
...
export async function applyFocusConstraints(stream: MediaStream): Promise<void> {
  try {
    ...
    if (!track) return;
    // focusMode is not yet in all TS lib typings for MediaTrackConstraintSet — cast via any
    await (track as any).applyConstraints({
      advanced: [{ focusMode: 'continuous' } as MediaTrackConstraintSet],
    });
  } catch {
    // Silently ignore — device or browser does not support focus constraints
  }
}
