...
import { applyFocusConstraints } from './utils/getMediaContraints';
...
export function useUserMedia(
  ...
  const getUserMedia = useCallback(async () => {
    ...
    // Apply continuous autofocus after the stream is obtained.
    // Silently ignored on browsers/devices that do not support this constraint (e.g. iOS < 17).
    await applyFocusConstraints(str);
    if (isMounted()) {
      setStream(str);
