...
| onGalleryPress                     | `() => void`                                 | Callback called when the user clicks on the gallery button. When this callback is provided, the gallery screen will be handled externally.                                                       |          |                                              |
...
| odometerUnit                       | `MileageUnit`                                | Global default mileage unit used when OCR cannot detect a unit from an odometer reading. Per-sight `defaultMileageUnit` in `ocrSights` takes precedence over this value.                        |          | `MileageUnit.KM`                             |
...
| videoUploadStrategy         | `VideoUploadStrategy`           | The strategy used to decide when a frame of the video should be selected and uploaded.                                                                                                            |          | `adaptiveUploadRate`      |
| frameSelectionInterval      | `number`                        | The interval (in milliseconds) at which frames are selected and uploaded when `videoUploadStrategy` is set to `fixedUploadRate`. Has no effect otherwise.                                        |          | `1000`                    |
| targetFramesCount           | `number`                        | The target number of frames to capture over a full 360° walkaround when `videoUploadStrategy` is set to `adaptiveUploadRate`. Has no effect otherwise. Clamped between 36 and 72.                 |          | `40`                      |
