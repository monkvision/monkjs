...
import { parseOdometerText } from './PhotoCaptureHUDOcrText.utils';
import {
  getCropRegion,
  getCropBoxStyle,
  getShutterHintStyle,
  ...
  getOverlayStyle,
  styles,
} from './PhotoCaptureHUDOcrOverlay.styles';
...
export interface PhotoCaptureHUDOcrOverlayProps {
  ...
  /** Returns the current camera frame as ImageData for live OCR processing. */
  getImageData: () => ImageData;
  /** When true, the camera is still initializing and frame capture should be skipped. */
  isCameraLoading: boolean;
  /** When false, the overlay renders nothing (non-OCR sight selected). */
  isActive: boolean;
...
/**
 * Camera overlay that drives live OCR on the current sight. It draws a crop-box frame over the
 * region of interest, runs the OCR model on successive frames until a reading is confirmed, then
 * shows a confirmation modal. When the retry or timeout limit is reached the shutter is unlocked
 * so the user can take a manual picture; OCR then runs on the crop of that picture instead.
 */
export function PhotoCaptureHUDOcrOverlay({
  config,
  getImageData,
  isCameraLoading,
  isActive,
  ...
  const { isReady, loadModels, processFrame, confirmedText, consistencyCount, reset, fatalError } =
    useOcr({
      ...ocrConfig,
      appearanceCount,
    });
  ...
  // Revoke the previous blob URL whenever ocrPicture changes or the overlay unmounts,
  // to avoid accumulating unreleased blob URLs across retries and confirmations.
  useEffect(() => {
    const uri = ocrPicture?.uri ?? null;
    return () => {
      if (uri) {
  ...
  // A fatal worker error (bad WASM URL, OOM, crash) permanently stops inference.
  // Treat it as an immediate timeout so the shutter unlocks instead of freezing the UI.
  useEffect(() => {
    if (fatalError) {
      setIsTimedOut(true);
    }
  ...
  useEffect(() => {
    ...
    canvasToBlob(canvas, mimetype, 0.92)
      ...
      .catch(() => {
        // Blob creation failed (e.g. CORS-tainted canvas, OOM). Reset so OCR can rescan
        // rather than leaving confirmedText set with no modal to surface it.
        reset();
      });
  }, [confirmedText, isFallbackReady, reset]);
  ...
  const handleConfirm = () => {
    ...
    // Prevent sending a malformed VIN: manual edits bypass the OCR stabilizer's
    // format check, so validate here before forwarding to the API.
  ...
  if (isConfirmed && confirmedText && !isEditing) {
    if (isOdometer) {
      const { value } = parseOdometerText(confirmedText);
      if (value !== null) {
    ...
    <>
      ...
      <div style={overlayStyle}>
        {isFallbackReady && !ocrPicture && (
          <div style={getShutterHintStyle(cropRegion)}>{t('photo.hud.ocr.shutterHint')}</div>
        )}
