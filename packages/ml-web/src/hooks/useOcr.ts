...
export function useOcr(config: UseOcrConfig): UseOcrResult {
  ...
  useEffect(() => {
    ...
    worker.onmessage = (e: MessageEvent<OcrWorkerResponse>) => {
      ...
      if (isSimilarText(text, prevRawRef.current, fuzzyTolerance)) {
        ...
        // Slide forward to the latest reading so VIN format validation uses the cleanest
        // value, not the stale first frame that may have contained OCR noise.
  ...
  const unloadModels = useCallback(() => {
    // Reset state immediately so any loadModels call from the next render passes the guard
    // before the worker-setup effect has run and called setIsReady(false) itself.
    setIsReady(false);
    setIsLoading(false);
    setFatalError(null);
    setWorkerKey((k) => k + 1);
