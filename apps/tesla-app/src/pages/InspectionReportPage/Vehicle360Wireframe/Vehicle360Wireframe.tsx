...
function createGetAttributesCallback(
  ...
  return (element, groups) => {
    ...
    const price = priceByPart?.find((p) => p.vehiclePart === part);
    if (price && price.price >= 0 && price.price < 300) {
      attributes.style = { ...attributes.style, fill: '#62b0ef' };
    }
    if (price && price.price >= 300 && price.price < 500) {
      attributes.style = { ...attributes.style, fill: '#e1a25b' };
    }
    if (price && price.price >= 500) {
      attributes.style = { ...attributes.style, fill: '#f49796' };
    }
...
export function Vehicle360Wireframe({
  ...
  return (
    <DynamicSVG
      ...
      getAttributes={createGetAttributesCallback(onClickPart, getPartAttributes, priceByPart)}
    />
  );
}
