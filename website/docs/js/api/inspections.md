...
| `limit`                        | number          | 100         |
| `onwershipFilter` | `own_resources` `organization_resources` `qall_resources` |     |
| `inspectionStatus`             | `NOT_STARTED` `IN_PROGRESS` `DONE` `ERROR` `VALIDATED`         |        |
...
> Please keep in mind that the **id** is mandatory in the **data** param, otherwise it will be considered as create one.
